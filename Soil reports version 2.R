      ...
      if (is.null(f$get_signed_in())) {
        ...
        sidebarLayout(
          sidebarPanel(
            useShinyjs(),
            # Input: Select a file ----
            hidden(div(id = "lab_data_input",textOutput("lab_warning"),
                       fileInput("file1", h5("Upload soil analysis results .csv"),
                                 accept = c(".csv")))),
          ...
          mainPanel(
            ...
            tabsetPanel( id = "tabs", type = "tabs",
                         tabPanel("Import data",
                                  br(),
                                  hidden(div(id = "Import_data", fluidRow(h4("Please upload a csv file")),
                                             fluidRow(a(href="csv_template.csv", "Download template csv file for data input", download=NA, target="_blank")),
                                             fluidRow(h5("Or access the google sheets version here to",a(href="https://docs.google.com/spreadsheets/d/1wZVuuK4sgG_EPaqxs6QA4PxPRxG7M8pvp_xp_s_sGK0/edit#gid=0", "make a copy", target="_blank"))))),
  ...
  observeEvent(input$soil_obs_data, {
    ...
    if (input$project_name != "" | input$project_year != "" | input$farm_name != "") {
      ...
      if (nrow(soil_obs) > 0) {
        ...
        if (input$project_year != "") {
          soil_obs <<- soil_obs[soil_obs$Project.year == input$project_year,]
        }
      ...
      observeEvent(input$file1, {
        ...
        field_data$Project_year_for_Duchy_only <- as.character(field_data$Project_year_for_Duchy_only)
        
        field_data <- field_data[field_data$Field_name %in% soil_obs_field_level$Field_name,]
        soil_obs_field_level <- soil_obs_field_level[soil_obs_field_level$Field_name %in% field_data$Field_name,]
        ...
        if (sum(field_data$Field_name == soil_obs_field_level$Field_name) == nrow(field_data)) {
          ...
          for (i in 1:nrow(field_data)) {
            field_data$Project_year_for_Duchy_only[i] <- ifelse(is.na(field_data$Project_year_for_Duchy_only[i]),1,field_data$Project_year_for_Duchy_only[i])
          }
          field_data <- field_data[order(field_data$Project_year_for_Duchy_only),]
          
          if (!is.null(field_data$Ag_stability_5mins1)) {
